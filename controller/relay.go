...
func executeTaskSubmissionWith(
	...
	for ; retryParam.GetRetry() <= common.RetryTimes; retryParam.IncreaseRetry() {
		...
		if !taskErr.LocalError {
			service.ReportChannelBreakerResult(channel.Id, channel.Name, taskAPIError)
	...
	task.PrivateData.BillingContext = &model.TaskBillingContext{
		...
		PerCallBilling:  relayInfo.PriceData.UsesPerCallBilling(common.StringsContains(constant.TaskPricePatches, relayInfo.OriginModelName)),
...
func decideTaskRetry(c *gin.Context, taskErr *taskdto.TaskError, retryTimes int) service.PolicyDecision {
	...
	// 与 DecideRelayRetry 一致：上游渠道级故障即使有会话亲和也允许切换渠道
	case (taskErr.LocalError || !operation_setting.ChannelBreakerMatchesStatusCode(taskErr.StatusCode)) && service.HandleChannelAffinityFailure(c):
