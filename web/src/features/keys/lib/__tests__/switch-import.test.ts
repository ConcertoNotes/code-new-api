...
describe('API key switch imports', () => {
  test('builds the VarSwitch protocol URL with provider details', () => {
    ...
    expect(parsed.protocol).toBe('varswitch:')
  ...
  test('uses the versioned endpoint for Codex imports', () => {
    ...
    expect(
      new URL(url).searchParams.get('endpoint')).toBe('https://api.example.com/v1'
    )
  })

  test('supports Grok imports in VarSwitch without changing CC Switch apps', () => {
    expect(SWITCH_IMPORT_APPS_BY_TARGET['var-switch']).toEqual([
      'claude',
      'codex',
      'gemini',
      'grok',
    ])
    expect(SWITCH_IMPORT_APPS_BY_TARGET['cc-switch']).toEqual([
      'claude',
      'codex',
      'gemini',
    ])
    ...
    expect(params.get('app')).toBe('grok')
    expect(params.get('model')).toBe('grok-4')
