...
export function captureQualityTestSVG(maxBytes: number): string {
  ...
  const localReference = (value: string): string => {
    ...
    if (
      url.hash &&
      (value.startsWith('#') ||
        url.href.split('#')[0] === new URL(document.baseURI).href.split('#')[0])
    ) {
  ...
  const localURLs = (value: string) =>
    value.replaceAll(
      /url\(\s*(?:"([^"]*)"|'([^']*)'|([^)]*?))\s*\)/gi,
      (_, double: string, single: string, bare: string) =>
        `url("${localReference(double ?? single ?? bare).replaceAll('"', '%22')}")`
    )
  const matrixText = (matrix: DOMMatrix) =>
  ...
  originals.forEach((original, index) => {
    ...
    for (const attribute of attributes) {
      if (/^on/i.test(attribute.localName) || attribute.localName === 'base') {
        ...
        attribute.value = localURLs(attribute.value)
      }
    }
    ...
    for (const name of [
      ...
      const animated = (
        original as unknown as Record<string, { animVal?: { value?: number } }>
      )[name]
    ...
    if (points?.numberOfItems) {
      copy.setAttribute(
        'points',
        Array.from({ length: points.numberOfItems }, (_, i) => {
    ...
    const parent =
      original.parentElement as unknown as SVGGraphicsElement | null
    if (
      original !== source &&
      original.localName !== 'svg' &&
      graphics.getScreenCTM &&
      parent?.getScreenCTM
    ) {
      const ownMatrix = graphics.getScreenCTM(),
        parentMatrix = parent.getScreenCTM()
    ...
    for (const name of ['gradientTransform', 'patternTransform']) {
      const transforms = (
        original as unknown as Record<string, SVGAnimatedTransformList>
      )[name]?.animVal
  ...
  clone
    .querySelectorAll(
      'script,style,animate,animateTransform,animateMotion,set,discard'
    )
  ...
  const width =
    box.width > 0
      ? box.width
      : source.width.animVal.value || candidates[0].rect.width
  const height =
    box.height > 0
      ? box.height
      : source.height.animVal.value || candidates[0].rect.height
  if (
    ![width, height].every(
...
function installQualityTestSVGBridge(
  previewID: string,
  capture: (limit: number) => string,
  limit: number
) {
  let loaded = false,
  ...
  const report = () => {
    ...
    available = hasSVG
    parent.postMessage(
      { type: 'quality-test-svg-ready', previewID, hasSVG },
      '*'
    )
  }
  const ready = () =>
  ...
  addEventListener('message', (event) => {
    ...
    if (
      event.source !== parent ||
      data?.type !== 'quality-test-svg-request' ||
    ...
    try {
      ...
      const code =
        ...
        ['noSVG', 'tooLarge', 'unsupported', 'notReady'].includes(error.message)
          ? error.message
          : 'failed'
      parent.postMessage(
        {
...
export function requestQualityTestSVG(
  frame: Window,
  previewID: string,
  signal: AbortSignal,
  host: Pick<Window, 'addEventListener' | 'removeEventListener'> = window,
  timeout = QUALITY_TEST_SVG_TIMEOUT
): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    const receive = (event: MessageEvent) => {
      ...
      if (
        event.source !== frame ||
        data?.type !== 'quality-test-svg-result' ||
        data.previewID !== previewID ||
      ...
      if (
        ...
        !data.svg ||
        data.svg.length > QUALITY_TEST_SVG_LIMIT
      ) {
        fail('invalidSVG')
        return
      }
    ...
    try {
      frame.postMessage(
        { type: 'quality-test-svg-request', previewID, requestID },
        '*'
      )
    } catch {
      fail('failed')
