...
export function RatioSettingsCard({
  ...
  useEffect(() => {
    modelNormalizedDefaults.current = {
      ModelPrice: normalizeJsonString(modelDefaults.ModelPrice),
      ImageGenerationPrice: normalizeJsonString(modelDefaults.ImageGenerationPrice),
      VideoGenerationPrice: normalizeJsonString(modelDefaults.VideoGenerationPrice),
      ModelRatio: normalizeJsonString(modelDefaults.ModelRatio),
      CacheRatio: normalizeJsonString(modelDefaults.CacheRatio),
      CreateCacheRatio: normalizeJsonString(modelDefaults.CreateCacheRatio),
      CompletionRatio: normalizeJsonString(modelDefaults.CompletionRatio),
    ...
    modelForm.reset({
      ...modelDefaults,
      ModelPrice: formatJsonForTextarea(modelDefaults.ModelPrice),
      ImageGenerationPrice: formatJsonForTextarea(modelDefaults.ImageGenerationPrice),
      VideoGenerationPrice: formatJsonForTextarea(modelDefaults.VideoGenerationPrice),
      ModelRatio: formatJsonForTextarea(modelDefaults.ModelRatio),
      CacheRatio: formatJsonForTextarea(modelDefaults.CacheRatio),
      CreateCacheRatio: formatJsonForTextarea(modelDefaults.CreateCacheRatio),
      CompletionRatio: formatJsonForTextarea(modelDefaults.CompletionRatio),
  ...
  const saveModelRatios = useCallback(
    async (values: ModelFormValues) => {
      ...
      // Local per-image/per-video prices and group expressions are stored as
      // plain options outside the versioned model pricing API.
      const extraOptionKeys = {
        ImageGenerationPrice: 'ImageGenerationPrice',
        VideoGenerationPrice: 'VideoGenerationPrice',
        GroupBillingExpr: 'billing_setting.group_billing_expr',
      } as const
      if (!pricingBaseline) return false
      try {
        ...
        const extraUpdates = (
          Object.keys(extraOptionKeys) as Array<keyof typeof extraOptionKeys>
        ).filter(
          (key) => normalized[key] !== modelNormalizedDefaults.current[key]
        )
        if (!changes.length && !visibilityChanged && !extraUpdates.length) {
        ...
        for (const key of extraUpdates) {
          const result = await updateOption.mutateAsync({
            key: extraOptionKeys[key],
