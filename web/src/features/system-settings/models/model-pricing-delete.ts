...
export function removeModelPricing(
  ...
  for (const field of modelPricingMapFields) {
    const valueMap = JSON.parse(values[field] || '{}') as Record<string, unknown>
    if (!valueMap || typeof valueMap !== 'object' || Array.isArray(valueMap)) throw new Error('Invalid JSON')
    delete valueMap[modelName]
    nextValues[field] = JSON.stringify(valueMap, null, 2)
  }

  const groups = JSON.parse(values.GroupBillingExpr || '{}') as Record<string, Record<string, string>>
  if (!groups || typeof groups !== 'object' || Array.isArray(groups)) throw new Error('Invalid JSON')
  for (const models of Object.values(groups)) {
    if (!models || typeof models !== 'object' || Array.isArray(models)) throw new Error('Invalid JSON')
