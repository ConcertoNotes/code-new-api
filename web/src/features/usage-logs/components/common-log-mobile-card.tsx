...
export function CommonLogMobileCard<TData>(props: {
  ...
  const clientCell = props.cells.get('client')
  ...
  return (
    <div className='min-w-0 space-y-2.5 text-sm leading-5'>
      ...
      {/* Admin-only column; absent for regular users */}
      {displayable && other?.client_user_agent && clientCell && (
        <div className='flex min-w-0 items-start gap-2 text-xs'>
          <span className='text-muted-foreground shrink-0'>
            {t('User Agent')}
          </span>
          <div className='min-w-0 flex-1 [&_[data-testid=client-user-agent-cell]]:max-w-none'>
            {flexRender(
              clientCell.column.columnDef.cell,
              clientCell.getContext()
            )}
          </div>
        </div>
      )}
