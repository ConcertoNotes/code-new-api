...
func SelectChannelForRequest(c *gin.Context, modelName string, retry *RetryParam) (*model.Channel, string, *ChannelSelectError) {
	...
	if retry.GetRetry() == 0 {
		if preferredChannelID, found := GetPreferredChannelByAffinity(c, modelName, usingGroup); found {
			...
			if err == nil && preferred != nil && preferred.Status == common.ChannelStatusEnabled && !IsChannelBreakerSkipping(preferred.Id) {
			...
			if affinityUsable {
				...
				upgraded := false
				if ShouldPreferHigherPriorityOverAffinity() {
					probe := *retry
					probe.Retry = common.GetPointer(0)
					candidate, candidateGroup, candidateErr := CacheGetRandomSatisfiedChannel(&probe)
				...
				if !upgraded {
					...
					MarkChannelAffinityUsed(c, selectGroup, preferred.Id)
				}
			} else if !ShouldKeepChannelAffinityOnChannelDisabled() {
				// 亲和渠道已禁用或冷却中：清除绑定后走正常选路，成功后会重新绑定到实际服务的渠道
