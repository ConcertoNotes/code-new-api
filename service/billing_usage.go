...
// shouldSuppressEstimatedClientGoneCharge identifies a streamed request for
// which the client disconnected before any upstream response was received and
// the usage was synthesized locally. In that case there is no upstream usage
// to reconcile against, so charging the local prompt estimate can bill a large
// request that never produced an upstream response.
func shouldSuppressEstimatedClientGoneCharge(c *gin.Context, relayInfo *relaycommon.RelayInfo) bool {
	if relayInfo == nil || !relayInfo.IsStream || relayInfo.StreamStatus == nil {
		return false
	}
	if relayInfo.StreamStatus.EndReason != relaycommon.StreamEndReasonClientGone || relayInfo.HasSendResponse() {
		return false
	}
	return common.GetContextKeyBool(c, constant.ContextKeyLocalCountTokens)
}
