...
func TestDecideRelayRetryReasons(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		// Channel failures (429/5xx/connection) still fail over under strict
		// sessions; strict only stops retries for non-channel errors.
		{name: "strict session", err: upstream(http.StatusUnauthorized), retries: 1, setup: func(c *gin.Context) {
