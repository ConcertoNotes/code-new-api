...
func IsRequestPolicyOption(key string) bool {
	if strings.HasPrefix(key, "channel_affinity_setting.") || strings.HasPrefix(key, "channel_breaker_setting.") {
		return true
	}
	switch key {
...
func BuildRequestPolicy(options map[string]string) (*RequestPolicySnapshot, error) {
	...
	for key, value := range raw {
		if field, ok := strings.CutPrefix(key, "channel_affinity_setting."); ok {
			switch field {
			case "enabled", "session_mode", "switch_on_success", "keep_on_channel_disabled", "prefer_higher_priority", "max_entries", "default_ttl_seconds", "rules":
			default:
