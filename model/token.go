...
type Token struct {
	...
	FallbackGroups     TokenFallbackGroups `json:"fallback_groups" gorm:"type:text"`
	AutoGroups         string              `json:"-" gorm:"type:text"`
	DeletedAt          gorm.DeletedAt      `gorm:"index"`
}

type TokenFallbackGroups string

func (groups TokenFallbackGroups) MarshalJSON() ([]byte, error) {
	return common.Marshal(groups.Values())
}

func (groups *TokenFallbackGroups) UnmarshalJSON(data []byte) error {
	var values []string
	if err := common.Unmarshal(data, &values); err != nil {
		return err
	}
	return groups.Set(values)
}

func (groups TokenFallbackGroups) Values() []string {
	if groups == "" {
		return []string{}
	}
	var values []string
	if err := common.UnmarshalJsonStr(string(groups), &values); err != nil {
		common.SysError("failed to parse token fallback groups: " + err.Error())
		return []string{}
	}
	return values
}

func (groups *TokenFallbackGroups) Set(values []string) error {
	if len(values) == 0 {
		*groups = ""
		return nil
	}
	...
	*groups = TokenFallbackGroups(data)
	return nil
}
...
func (token *Token) Update() (err error) {
	...
	return DB.Model(token).Select("name", "status", "expired_time", "remain_quota", "unlimited_quota",
		"model_limits_enabled", "model_limits", "allow_ips", "group", "cross_group_retry", "fallback_groups", "auto_groups").Updates(token).Error
}
