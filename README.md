...
|--------|------|--------|
| `SESSION_SECRET` | Authentication signing secret; must be identical on every node. When unset, one is auto-generated and persisted to `./session.secret` (`/data/session.secret` in the container) so logins survive restarts and rebuilds | Auto-generated and persisted |
